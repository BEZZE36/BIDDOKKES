...
- Name: admin-flow.spec.js >> Admin Login Page >> should display login page
- Location: tests\admin-flow.spec.js:23:3
...
Locator: locator('h1, h2').filter({ hasText: /login|masuk|admin/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('h1, h2').filter({ hasText: /login|masuk|admin/i }).first()
